...
import { runExternalCLI, cliResultToMessage } from '@/core/agent-cli/runner';
...
export default function ChatView() {
  ...
  const handleSend = async (text: string, images?: ImageAttachment[], workspacePath?: string | null) => {
    // Check if an external CLI is active — route through CLI runner instead of abu agent loop
    const activeCLIInstance = useAgentCLIStore.getState().getActiveInstance();
    if (activeCLIInstance) {
      ...
      try {
        const result = await runExternalCLI(activeCLIInstance, text);
        const { assistantMessage } = cliResultToMessage(activeCLIInstance, text, result);
        useChatStore.getState().addMessage(convId, {
          ...assistantMessage,
          id: generateId(),
        } as Message);
        useChatStore.getState().setConversationStatus(convId, 'idle');
      } catch (err) {
        useChatStore.getState().addMessage(convId, {
          id: generateId(),
          role: 'assistant',
          content: `**${activeCLIInstance.label} 执行失败**\n\`\`\`\n${String(err)}\n\`\`\``,
          timestamp: Date.now(),
        });
        useChatStore.getState().setConversationStatus(convId, 'idle');
      }
